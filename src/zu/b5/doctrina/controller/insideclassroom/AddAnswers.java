...
public class AddAnswers extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
        ...
        if(count == answersArray.size()) {
            ...
            try {
                String table = tableName.substring(0, tableName.length()-6) + "s";
                stmt = conn.prepareStatement("select id from "+table+" where question_id = "+id+";");
                ResultSet rs = stmt.executeQuery();
                ArrayList<String> title_id = get.resultSetToUserID(rs);
                
                String status = tableName.substring(0,tableName.length()-6)+"status";
                stmt = conn.prepareStatement("insert into "+status+ " values(?, ?, ?);");
